...
assert(html.includes('id="play-loading"'), 'PLAY loading overlay');
assert(html.includes('id="play-loading-fill"'), 'PLAY loading bar fill');
assert(html.includes('id="btn-play"'), 'PLAY button stays');
assert(uiSrc.includes('showPlayLoading'), 'showPlayLoading');
assert(gameSrc.includes('_warmupForPlay'), 'PLAY warms shaders before fight');
assert(gameSrc.includes('compileAsync') || gameSrc.includes('compile('), 'shader compile during warmup');
